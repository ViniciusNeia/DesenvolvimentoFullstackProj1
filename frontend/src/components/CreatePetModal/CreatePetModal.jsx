...
function CreatePetModal({ isOpen, onClose }) {
  ...
  useEffect(() => {
    ...
    const found = breedsList.find(
      (b) => String(b.id) === String(selectedBreedId) || b.name === selectedBreedId
    );
  ...
  async function handleSubmit(e) {
    ...
    if (!name || !selectedBreedId) {
      return showPopupModal(
        "Campos faltando",
        "Nome e raça são obrigatórios",
        "warning"
      );
    }
    ...
    try {
      ...
      try {
        await loadCreatedPets();
      } catch {}

      try {
        window.dispatchEvent(
          new CustomEvent("createdPetAdded", { detail: created })
        );
      } catch {}
          ...
          <form onSubmit={handleSubmit} className={styles.form}>
            ...
            {breedDetails && (
              <div className={styles.breedDetails}>
                <h4>{breedDetails.name}</h4>
                {breedDetails.temperament && (
                  <p>
                    <strong>Temperamento:</strong> {breedDetails.temperament}
                  </p>
                )}
                ...
                {breedDetails.origin && (
                  <p>
                    <strong>Origem:</strong> {breedDetails.origin}
                  </p>
                )}
              </div>
            )}
